...
GuiMenu::GuiMenu(Window* window) : GuiComponent(window), mMenu(window, "MAIN MENU"), mVersion(window)
{
    ...
    if(system_has_bluetooth) {  // only create this entry if System Bluetooth is set
        addEntry("BLUETOOTH", 0x777777FF, true, 
            [this] {
                auto s = new GuiSettings(mWindow, "BLUETOOTH");

                Window* window = mWindow;

                ComponentListRow row;
                row.makeAcceptInputHandler([window] { GuiBluetoothListDevices(window); });
                row.addElement(std::make_shared<TextComponent>(window, "LIST DEVICES", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
                s->addRow(row);

                row.elements.clear();
                row.makeAcceptInputHandler([window] { GuiBluetoothScanDevices(window); });
                row.addElement(std::make_shared<TextComponent>(window, "SCAN DEVICES", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
                s->addRow(row);

                row.elements.clear();
                row.makeAcceptInputHandler([window] { GuiBluetoothRemoveDevices(window); });
                row.addElement(std::make_shared<TextComponent>(window, "REMOVE DEVICES", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
                s->addRow(row);

                if(system_has_clear_bt_cache) {
                    row.elements.clear();
                    row.makeAcceptInputHandler([window] { GuiBluetoothClearCache(window); });
                    row.addElement(std::make_shared<TextComponent>(window, "CLEAR CACHE", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
                    s->addRow(row);
                }
...
#ifdef AARCH64
            if(system_has_reboot_overclock) {
                row.elements.clear();
                row.makeAcceptInputHandler([window] {
                    window->pushGui(new GuiMsgBox(window, "REALLY REBOOT OVERCLOCK?", "YES", 
                    [] { 
                        if(runRebootOverclock() != 0)
                            LOG(LogWarning) << "Reboot overclock terminated with non-zero result!";
                    }, "NO", nullptr));
                });
                row.addElement(std::make_shared<TextComponent>(window, "REBOOT OVERCLOCK", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
                s->addRow(row);
            }
#endif
